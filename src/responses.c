...
char *http_get_file_content(const char *filename) {
...
char *http_viewname_to_filename(const char *view_name) {
    char *filename = malloc(strlen(view_name) + 8);
    strcpy(filename, "./views/");
    strncpy(filename + strlen(filename), view_name, strlen(view_name));
    strcat(filename, ".html");
    return filename;
}

char *http_render_template(const char *view_name, const hashmap_map *context) {
    (void) context; // Unused parameter (for now)
